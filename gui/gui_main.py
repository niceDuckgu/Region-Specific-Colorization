...
class UclUi(QWidget):
    def __init__(self, color_model, img_file=None, load_size=224, win_size=256, device='cpu' , args=None):
        # draw the layout
        self.args = args
        ...
        self.drawWidget = GUIDraw(color_model, load_size=load_size, win_size=win_size, device=device, args=self.args, time=self.start_t)
        ...
        self.bLocalization = QCheckBox('&Localization')
        self.bLocalization.setToolTip('Activate localization tool')
        drawPadMenu.addWidget(self.bLocalization)
        self.bLocalization.clicked.connect(self.toggleLocalization)
        ...
        # self.localization.toggled.connect(self.enable_localization)
    ...
    def toggleLocalization(self):
        self.drawWidget.toggleLocalization()
        self.bLocalization.setChecked(self.drawWidget.localizationActive)
        self.update()  
    ...
    #     self.drawWidget.enable_localization()
    ...
    def save(self):
        time_spend = time.time() - self.start_t
        print('time spent = %3.3f' % (time_spend))

        # print('time spent = %3.3f' % (time.time() - self.start_t))
        #TODO 
        self.drawWidget.save_result(time_spend)
