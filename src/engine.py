...
import utils
from losses import HuberLoss, PatchAwareLoss
...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    ...
                    wd_schedule_values=None, exp_name=None, patch_loss=False, beta=0.1):
    model.train()
    ...
    for step, (batch, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        images = batch['image']
        bool_hinted_pos = batch['mask']
        cropped_hint_l = batch['cropped_hint']
        hint_ratio = batch['crop_ratio']
        hint_loc = batch['hint_loc']
        ...
        bool_hinted_pos = ~ bool_hinted_pos # important 
        ...
        with torch.cuda.amp.autocast():
            outputs = model(images_l, bool_hinted_pos, cropped_hint_l, hint_mask)
            ...
            if patch_loss:
                ...
                loss = loss_func(input=outputs, target=labels[:, :, :, 1:]) \
                    + beta*loss_func_2(input=outputs, target=labels[:, :, :, 1:], hint_mask=hint_mask)
...
def validate(model: torch.nn.Module, data_loader: Iterable, device: torch.device,
    ...
    with torch.no_grad():
        for step, batch in tqdm(enumerate(data_loader), desc=header, ncols=100, total=len(data_loader)):
            ...
            cropped_hint_l = batch['cropped_hint']
            # TODO from images, get the hint ab values
            ...
            for idx, count in enumerate(val_hint_list):
                ...
                bool_hint = ~bool_hint # 
                with torch.cuda.amp.autocast():
                    outputs = model(images_l.clone(), bool_hint.clone(), cropped_hint_l[:, idx], hint_mask[:, idx])
                    outputs = rearrange(outputs, 'b n (p1 p2 c) -> b n (p1 p2) c', p1=patch_size, p2=patch_size)
