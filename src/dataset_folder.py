...
class DatasetFolder(VisionDataset):
    ...
    def __init__(
        ...
        # Prefer cached samples if available inside the repository; otherwise build on the fly
        cache_dir = Path(__file__).resolve().parent / "samples"
        cache_path = cache_dir / f"{osp.basename(root)}_samples.pkl"
...
class ImageWithFixedHint(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        output = self.transform(img, hint_coords)
        img = output['image']
        hint_coords = output['mask']
        target = 0
        if self.return_name:
             output['name'] = self.img_list[idx]
        return output
